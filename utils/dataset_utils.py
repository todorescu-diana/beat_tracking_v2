...
def load_dataset(dataset_name, strict=False, gtzan=False, tiny_aam=False):
...
def find_annotation_file(annotations_dir, track_id, strict=False, gtzan=False, tiny_aam=False):
    # iterate through files and directories in annotations directory
    for item in os.listdir(annotations_dir):
        # if strict is False and track_id in item:
        ...
        if os.path.isdir(item_path):
            ...
            if track_id in item:
                ...
                annotation_file = find_annotation_file(item_path, track_id, gtzan)
        ...
        # if it's a file and contains the track ID in its name, return its path
            ...
            if tiny_aam is True and track_id.split('_')[0] == item.split('_')[0] and 'beatinfo' in item:
                return item_path
            elif gtzan is True and track_id.replace('.', '_') in item.split('.')[0]:
                return item_path
            elif (strict is False and (track_id in item.split('.')[0] or item.split('.')[0] in track_id)) or (strict is True and track_id == item.split('.')[0]):
                return item_path
